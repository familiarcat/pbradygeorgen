...
export class CursorIntegration {
  ...
  private async sendToCursorAI(message: string, context: SharedContext): Promise<AIResponse | null> {
  ...
  private buildCursorPrompt(message: string, context: SharedContext, editorContext: any): string {
...
Response as Cursor AI:`;
  ...
  private async simulateCursorResponse(
    prompt: string,
    ...
    const response = this.generateResponseByType(
      this.classifyMessageType(prompt), 
      prompt, 
      { filename: editorContext.fileName, language: editorContext.language }, 
      editorContext.currentCode
    );
  ...
  private generateIntelligentMockResponse(message: string, context: SharedContext): AIResponse {
  ...
  private generateResponseByType(
    messageType: string,
    message: string,
    activeFile: any,
    selectedCode?: string
...
What specific action would you like me to take first?`;
  ...
  private generateCodeSample(message: string, language?: string): string {
  ...
  try {
    ...
    return samples[lang as keyof typeof samples] || samples.typescript;
  }

  private generateRefactoredCode(originalCode: string, language?: string): string {
  ...
  private generateFileList(language?: string): string {
    const files = {
    ...
    const fileList = files[language as keyof typeof files] || files.default;
  ...
  private parseCursorResponse(responseText: string, aiSource: 'cursor', editorContext: any): AIResponse {
    // Extract code blocks
  ...
  private extractFileSuggestions(responseText: string, language?: string): string[] {
    ...
    const pattern = patterns[language as keyof typeof patterns] || patterns.default;
  ...
  async handleRealTimeEdit(
    editType: 'autocomplete' | 'refactor' | 'debug',
    currentContext: any,
