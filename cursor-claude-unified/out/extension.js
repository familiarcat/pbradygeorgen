...
function activate(context) {
    ...
    const enhanceFileContext = vscode.commands.registerCommand('cursor-claude.enhanceFileContext', async () => {
        await injectEnhancedFileContext();
    });
    // 2. PROVIDE code generation suggestions for Cursor's AI
    const provideCodeSuggestions = vscode.commands.registerCommand('cursor-claude.provideCodeSuggestions', async () => {
        await generateCodeSuggestions();
    });
    // 3. ANALYZE workspace for Cursor's AI context
    const analyzeWorkspace = vscode.commands.registerCommand('cursor-claude.analyzeWorkspace', async () => {
        await analyzeWorkspaceForCursor();
    });
    // 4. INTEGRATE with N8N workflows for enhanced AI capabilities
    const integrateN8N = vscode.commands.registerCommand('cursor-claude.integrateN8N', async () => {
        await integrateWithN8NWorkflows();
    });
    // 5. SHOW enhanced context status
    const showContextStatus = vscode.commands.registerCommand('cursor-claude.showContextStatus', async () => {
        await showEnhancedContextStatus();
    });
    // 6. TOGGLE enhancement features
    const toggleEnhancement = vscode.commands.registerCommand('cursor-claude.toggleEnhancement', async () => {
        await toggleEnhancementFeatures();
    });
    // 7. QUICK file analysis for Cursor's AI
    const quickFileAnalysis = vscode.commands.registerCommand('cursor-claude.quickFileAnalysis', async () => {
        await performQuickFileAnalysis();
    });
    // 8. ENHANCE current selection for Cursor's AI
    const enhanceSelection = vscode.commands.registerCommand('cursor-claude.enhanceSelection', async () => {
        await enhanceCurrentSelection();
    });
    // 9. SHOW integration insights
    const showInsights = vscode.commands.registerCommand('cursor-claude.showInsights', async () => {
        await showIntegrationInsights();
    });
    // 10. NEW: Send task to N8N for unified AI routing
    const sendTaskToN8N = vscode.commands.registerCommand('cursor-claude.sendTaskToN8N', async () => {
        await sendTaskToUnifiedN8NSystem();
    });
    // 11. NEW: Show real-time LLM selection status
    const showLLMStatus = vscode.commands.registerCommand('cursor-claude.showLLMStatus', async () => {
        await showRealTimeLLMStatus();
    });
    // 12. NEW: Show cost optimization dashboard
    const showCostOptimization = vscode.commands.registerCommand('cursor-claude.showCostOptimization', async () => {
        await showCostOptimizationDashboard();
    });
    // Register all commands
    context.subscriptions.push(enhanceFileContext, provideCodeSuggestions, analyzeWorkspace, integrateN8N, showContextStatus, toggleEnhancement, quickFileAnalysis, enhanceSelection, showInsights, sendTaskToN8N, showLLMStatus, showCostOptimization);
    // Show activation message
    vscode.window.showInformationMessage('🚀 Cursor AI Chat Extender activated! Use Command Palette to enhance Cursor\'s AI chat.');
...
async function injectEnhancedFileContext() {
...
async function analyzeWorkspaceForCursor() {
...
// 4. Integrate with N8N workflows for enhanced AI capabilities
async function integrateWithN8NWorkflows() {
    try {
...
// 5. Show enhanced context status
async function showEnhancedContextStatus() {
...
// 6. Toggle enhancement features
async function toggleEnhancementFeatures() {
...
async function performQuickFileAnalysis() {
    const activeEditor = vscode.window.activeTextEditor;
    if (!activeEditor) {
...
// 8. Enhance current selection for Cursor's AI
async function enhanceCurrentSelection() {
    const activeEditor = vscode.window.activeTextEditor;
    if (!activeEditor) {
...
// 9. Show integration insights
async function showIntegrationInsights() {
...
// 10. NEW: Send task to N8N for unified AI routing
async function sendTaskToUnifiedN8NSystem() {
    try {
...
// 11. NEW: Show real-time LLM selection status
async function showRealTimeLLMStatus() {
    try {
...
async function sendTaskToN8NWebhook(taskData) {
...
async function showEnhancedAIResponse(response) {
...
async function getN8NWorkflowStatus() {
...
async function analyzeFileStructure(document) {
...
async function generateCodeSuggestionsForSelection(code, language) {
...
async function analyzeWorkspaceStructure(workspacePath) {
...
async function updateFileContextForCursor(uri) {
...
async function setEnhancementState(enabled) {
...
async function performQuickAnalysis(document) {
    const text = document.getText();
    const lines = text.split('\n');
    return {
...
async function enhanceSelectionForCursor(selectedText, language) {
    return {
...
async function showCostOptimizationDashboard() {
    try {
...
💡 **Tip:** Your AI costs are automatically optimized for every task!`;
        ...
        const selectedAction = await vscode.window.showInformationMessage('💰 Cost optimization dashboard ready! Choose how to view it:', copyAction, showInChatAction, showDetailedAction);
