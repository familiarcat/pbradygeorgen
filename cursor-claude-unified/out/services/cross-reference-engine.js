...
exports.CrossReferenceEngine = void 0;
const claude_integration_1 = require("./claude-integration");
const cursor_integration_1 = require("./cursor-integration");
class CrossReferenceEngine {
    constructor() {
        this.claudeIntegration = new claude_integration_1.ClaudeIntegration();
        this.cursorIntegration = new cursor_integration_1.CursorIntegration();
    }
    async generateUnifiedResponse(selection, userMessage, context) {
        // 1. Generate primary AI response
        const primaryResponse = await this.generatePrimaryResponse(selection.primary_ai, userMessage, context);
        // 2. Generate secondary AI enhancement with cross-references
        const secondaryResponse = await this.generateSecondaryEnhancement(selection.secondary_ai, selection.primary_ai, primaryResponse, userMessage, context);
    ...
    async generatePrimaryResponse(ai, userMessage, context) {
    ...
    async generateSecondaryEnhancement(secondaryAI, primaryAI, primaryResponse, userMessage, context) {
    ...
    buildEnhancedPrompt(ai, userMessage, context, isPrimary) {
        const aiPersonality = {
            cursor: {
    ...
    extractCrossReferences(primaryResponse, secondaryResponse) {
        ...
        mentions.forEach(mention => {
            const mentionedAI = mention.toLowerCase().replace('@', '');
    ...
    generateTaskAnalysis(userMessage, context) {
        const activeFile = context.file_context.find(f => f.is_active);
    ...
    calculateTotalCost(primaryResponse, secondaryResponse, selection) {
    ...
    async handleCollaborativeEdit(editRequest, currentCode, context) {
        const cursorPrompt = `
...
exports.CrossReferenceEngine = CrossReferenceEngine;
//# sourceMappingURL=cross-reference-engine.js.map