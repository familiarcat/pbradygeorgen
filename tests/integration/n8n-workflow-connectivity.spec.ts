...
test.describe('N8N Workflow Connectivity', () => {
    test('should diagnose mission scenario endpoint failures', async ({ page }) => {
        ...
        const mostCommonStatus = statusCodes.reduce((a, b, i, arr) => 
        ...
        expect(statusCodes.every(code => code !== 200)).toBe(true); // Confirms all are failing
    });
    ...
    test('should verify N8N webhook connectivity', async ({ page }) => {
        ...
        expect(successfulWebhooks.length).toBeGreaterThan(failedWebhooks.length);
    });

    test('should test N8N workflow execution flow', async ({ page }) => {
    ...
    test('should verify N8N error handling and recovery', async ({ page }) => {
    ...
    test('should test N8N workflow performance patterns', async ({ page }) => {
        ...
        expect(successCount).toBeGreaterThanOrEqual(performanceResults.length * 0.8);
    });
});