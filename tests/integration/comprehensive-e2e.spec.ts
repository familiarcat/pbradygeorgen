...
test.describe('Comprehensive End-to-End Tests', () => {
    test('should execute complete mission workflow from UI to API', async ({ page }) => {
        ...
        if (await testingLink.isVisible()) {
            ...
            if (await picardCard.isVisible()) {
                ...
                expect(hasSuccess || hasError).toBe(true); // Should show some result
            }
        } else {
    ...
    test('should test cross-system data persistence', async ({ page }) => {
        ...
        expect([200, 404]).toContain(createResponse.status());
    });

    test('should test system recovery after failures', async ({ page }) => {
        console.log('🛠️ Testing system recovery capabilities...');
        
        // Test scenario: System under stress
        const stressTestRequests = [];
        for (let i = 0; i < 5; i++) {
            stressTestRequests.push(
                page.request.post('/api/test-n8n/crew-member', {
                    data: {
                        crewMemberId: 'data',
    ...
    test('should validate complete user journey', async ({ page }) => {
    ...
    test('should test system monitoring and health checks', async ({ page }) => {
    ...
    test('should generate comprehensive system report', async ({ page }) => {
        ...
        const operationalComponents = Object.values(systemReport.components)
            .filter((comp: any) => comp.operational).length;
        ...
        systemReport.performance.averageResponseTime = Object.values(systemReport.components)
            .reduce((sum: number, comp: any) => sum + comp.responseTime, 0) / totalComponents;
        ...
        expect(systemReport.performance.overallHealth).toBeGreaterThan(50);
        expect(systemReport.performance.averageResponseTime).toBeLessThan(10000);
