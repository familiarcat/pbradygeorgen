...
export async function analyzeColorsWithOpenAI(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const apiError = new Error(`OpenAI API error: ${response.status}`);

      try {
        // Try to parse the error response as JSON
        const errorData = await response.json();

        // Add the error data to the error object
        ...
        return handleOpenAIError(apiError, 'Color Analysis');
      } catch (parseError) {
        // If we can't parse the error as JSON, try to get the raw text
        try {
          const errorText = await response.text();

          // Add the error text to the error object
          (apiError as any).text = errorText;
    ...
    // Use our centralized error handler
    handleOpenAIError(error, 'Color Analysis');
