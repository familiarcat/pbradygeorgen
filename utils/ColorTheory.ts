...
import { ColorTheme, defaultColorTheme } from './SimplePDFColorExtractor';
...
export class ColorTheory {
  ...
  static generateCtaColors(primaryColor: string): SalingerCtaColors {
    try {
      ...
      if (!HesseColorTheory) {
        ...
        try {
          DanteLogger.warn.runtime('HesseColorTheory is undefined, using fallback');
        } catch (logError) {
          // Silent catch
        }
      ...
      DanteLogger.error.runtime(`Error generating CTA colors: ${error}`);
  ...
  static generateColorPalette(
    ...
    try {
      ...
      DanteLogger.error.runtime(`Error generating color palette: ${error}`);
  ...
  static createFallbackTheme(rawColors: string[]): ColorTheme {
    try {
      ...
      DanteLogger.error.runtime(`Error creating fallback theme: ${error}`);
