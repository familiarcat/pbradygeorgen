  ...
  try {
    ...
    const retryWithBackoff = async (attempt: number): Promise<any> => {
      try {
        ...
        // Log the error
        DanteLogger.error.runtime(`Error during OpenAI color analysis (attempt ${attempt}/${maxRetries}): ${errorMessage}`);
        ...
        if (attempt >= maxRetries) {
          ...
          // Wrap in Error object if it's not already one
    ...
    try {
      ...
      // Safely convert error to string to avoid "Cannot read properties of undefined" errors
      const errorMessage = analyzeError instanceof Error ? analyzeError.message : String(analyzeError || 'Unknown error');

      // Use PhilosophicalLogger instead of DanteLogger directly for better error handling
      try {
        DanteLogger.error.runtime(`Error during OpenAI color analysis after ${maxRetries} attempts: ${errorMessage}`);
      } catch (logError) {
        // Fallback to console if logger fails
        console.error(`Error during OpenAI color analysis after ${maxRetries} attempts: ${errorMessage}`);
      }

      console.error('OpenAI color analysis failed after multiple attempts:', analyzeError || 'Unknown error');
    ...
    // Use try-catch for logging to prevent additional errors
    try {
      DanteLogger.error.runtime(`Failed to extract colors from PDF: ${errorMessage}`);
    } catch (logError) {
      // Fallback to console if logger fails
      console.error(`Failed to extract colors from PDF: ${errorMessage}`);
    }

    console.error('Failed to extract colors from PDF:', error || 'Unknown error');

    // Use the createFallbackColorTheme function with an empty array of colors
    // This will use the default color theme
    let ultimateFallbackTheme;
    try {
      ultimateFallbackTheme = ColorTheory.createFallbackTheme([]);
    } catch (fallbackError) {
      console.error('Error creating fallback theme:', fallbackError || 'Unknown error');
      // Return the default color theme as a last resort
      ultimateFallbackTheme = { ...defaultColorTheme, rawColors: [], isLoading: false };
    }
    ...
    try {
      ...
      // Fallback to console if logger fails
      console.warn(`Using ultimate fallback color theme due to extraction error`);
    }
