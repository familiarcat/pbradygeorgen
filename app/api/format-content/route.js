...
async function POST(request) {
    ...
    try {
        ...
        try {
            ...
            schemas_1.FormatContentResponseSchema.parse(errorResponse);
        }
        ...
        return server_1.NextResponse.json(errorResponse, { status: 400 });
    }
    // Validate request against our schema
    const parseResult = schemas_1.FormatContentRequestSchema.safeParse(data);
    if (!parseResult.success) {
        // Extract validation errors from Zod
        const errorMessage = parseResult.error.errors.map(err => `${err.path.join('.')}: ${err.message}`).join(', ');
        const errorResponse = {
            success: false,
        ...
        try {
            ...
            // Even Zod has failed us! Log the validation error but continue
            if (validationError instanceof Error) {
                DanteLogger_1.DanteLogger.error.validation(`Validation failed: ${validationError.message}`);
            }
        }
        return server_1.NextResponse.json(errorResponse, { status: 400 });
    }
    ...
    try {
        ...
        try {
            ...
            // Even Zod has failed us! Log the validation error but continue
            if (validationError instanceof Error) {
                DanteLogger_1.DanteLogger.error.dataFlow(`Validation failed: ${validationError.message}`);
            }
        }
        return server_1.NextResponse.json(errorResponse, { status: 500 });
    }
    ...
    DanteLogger_1.DanteLogger.success.basic(`Content type detected: ${contentType}`);
    ...
    if (format === 'markdown') {
        ...
        DanteLogger_1.DanteLogger.success.ux(`Content formatted as markdown successfully`);
    }
    else if (format === 'text') {
        ...
        formattedContent = result.data;
        DanteLogger_1.DanteLogger.success.ux(`Content formatted as text successfully`);
    }
    ...
    try {
        ...
        schemas_1.FormatContentResponseSchema.parse(successResponse);
        DanteLogger_1.DanteLogger.success.perfection(`Content formatting completed successfully for ${format} format`);
        return server_1.NextResponse.json(successResponse);
    }
    catch (error) {
        // Handle Zod validation errors with Salinger-inspired lightheartedness
        if (error instanceof Error && error.name === 'ZodError') {
            DanteLogger_1.DanteLogger.error.config(`Response validation failed: ${error.message}`);
        }
        else {
            DanteLogger_1.DanteLogger.error.system('Error validating response format', error);
        }
 ...
 */
async function analyzeContentType(content) {
    try {
        // Check if OpenAI API key is available
        if (!isOpenAIKeyAvailable()) {
            DanteLogger_1.DanteLogger.warn.deprecated('OpenAI API key is not available, using default content type');
        ...
        const contentType = schemas_1.ContentTypeSchema.parse(rawContentType);
        DanteLogger_1.DanteLogger.success.core(`Content type validated by Zod: ${contentType}`);
        return { success: true, data: contentType };
    }
    catch (error) {
        DanteLogger_1.DanteLogger.error.corruption('Error analyzing content type', error);
        // If it's a Zod error, we got an invalid content type from OpenAI
        if (error instanceof Error && error.name === 'ZodError') {
            DanteLogger_1.DanteLogger.error.validation('Invalid content type received from OpenAI, defaulting to "other"');
            return {
                success: true,
                data: 'other',
 ...
 */
async function formatContentAsMarkdown(content, contentType) {
    ...
    try {
        ...
        if (true) {
            DanteLogger_1.DanteLogger.warn.deprecated('Using custom resume formatting for consistency');
 ...
 */
async function formatContentAsText(content, contentType) {
    // Create a prompt based on the content type
    ...
    try {
        // Always use our own formatting for consistency
        if (true) {
            DanteLogger_1.DanteLogger.warn.deprecated('Using custom resume text formatting for consistency');
