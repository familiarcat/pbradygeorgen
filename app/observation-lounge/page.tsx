...
export default function ObservationLoungePage() {
  ...
  const testCrewMember = async (crewId: string) => {
    setIsLoading(true);
    setCrewMembers(prev => 
      prev.map(crew => 
        crew.id === crewId 
          ? { ...crew, status: 'testing' as const }
          : crew
      )
    );
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <ul className="space-y-2">
                {meetingResults.top_priorities.map((priority, index) => (
            ...
            <div>
              ...
              <ul className="space-y-2">
                {meetingResults.next_steps.slice(0, 3).map((step, index) => (
                  <li key={index} className="flex items-start">
        ...
        <div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
          {crewMembers.map((crew) => (
            <div key={crew.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow duration-200">
              <div className="p-6">
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => testCrewMember(crew.id)}
                    disabled={isLoading || crew.status === 'testing'}
                ...
                {selectedCrew === crew.id && crew.analysis && (
