#!/bin/bash
# Script to test the PDF extraction process
...
# Print test header
echo -e "\n${BOLD}${CYAN}=== PDF Extraction Test ===${NC}\n"
print_message "${BLUE}" "INFO" "This test will verify the PDF extraction process"
...
# Step 5: Test the PDF extraction scripts individually
print_message "${CYAN}" "STEP 5" "Testing individual extraction scripts"

# Test text extraction
if ! run_command "node scripts/extract-pdf-text-improved.js $DEFAULT_PDF" "Testing text extraction"; then
  print_message "${RED}" "ERROR" "Text extraction failed"
  exit 1
fi

# Test color extraction
if ! run_command "node scripts/extract-pdf-colors.js $DEFAULT_PDF" "Testing color extraction"; then
  print_message "${YELLOW}" "WARNING" "Color extraction failed, but continuing"
fi

# Test font extraction
if ! run_command "node scripts/extract-pdf-fonts.js $DEFAULT_PDF" "Testing font extraction"; then
  print_message "${YELLOW}" "WARNING" "Font extraction failed, but continuing"
fi

# Test markdown generation
if ! run_command "node scripts/generate-improved-markdown.js $EXTRACTED_DIR/resume_content.txt" "Testing markdown generation"; then
  print_message "${YELLOW}" "WARNING" "Markdown generation failed, but continuing"
fi

# Step 6: Verify the extraction results
print_message "${CYAN}" "STEP 6" "Verifying extraction results"

# Check if the text content is not empty
if ! run_command "[ -s $EXTRACTED_DIR/resume_content.txt ]" "Checking if text content is not empty"; then
  print_message "${RED}" "ERROR" "Text content is empty"
  exit 1
fi

# Check if the markdown content is not empty
if ! run_command "[ -s $EXTRACTED_DIR/resume_content.md ]" "Checking if markdown content is not empty"; then
  print_message "${RED}" "ERROR" "Markdown content is empty"
  exit 1
fi

# Print success message
print_message "${GREEN}" "SUCCESS" "PDF extraction test completed successfully"
print_message "${BLUE}" "INFO" "All extraction scripts are working correctly"
