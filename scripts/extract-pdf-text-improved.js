...
async function main() {
  ...
  try {
    ...
    if (!text || text.includes('[object Object]')) {
      ...
      selectedPersona = philosophicalPersonas[random(0, philosophicalPersonas.length)];
      console.log(`Selected philosophical persona: ${selectedPersona.name}`);
    ...
    const jsonContent = {
      metadata: {
        ...
        selectedPersona: selectedPersona ? selectedPersona.name : null
      },
      rawText: text,
      sections: sections,
    ...
    const jsonPath = path.join(path.dirname(outputPath), 'resume_content.json');
    ...
    console.log(`JSON content saved to: ${jsonPath}`);
