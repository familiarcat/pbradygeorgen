#!/bin/bash
# Script to run the Husserl's Spirit dialogue with the PDF-styling reincarnation prompt
...
if [ ! -f "prompts/reincarnation-pdf-styling.md" ]; then
  echo "❌ Error: PDF styling reincarnation prompt not found"
  echo "Please make sure the file prompts/reincarnation-pdf-styling.md exists"
  exit 1
fi
...
const promptPath = path.join(process.cwd(), 'prompts', 'reincarnation-pdf-styling.md');
...
console.log('🧠 Starting conversation with PDF-Styling Spirit...');
...
async function startConversation() {
  ...
  console.log('AI: Initializing PDF-Styling Spirit...');
  const initialResponse = await sendMessage('Hello, I\'d like to learn about PDF-driven styling in AlexAI.');
...
# Run the script
echo "🧠 Starting Husserl's Spirit dialogue with PDF-Styling reincarnation..."
node "$TMP_SCRIPT"
