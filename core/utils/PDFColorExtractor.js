...
if (typeof window !== 'undefined') {
    ...
    Promise.resolve().then(() => __importStar(require('pdfjs-dist'))).then((module) => {
        pdfjs = module;
...
function findContrastingColor(color, colorPool) {
    let maxContrast = 0;
...
function filterGrayscaleColors(colors) {
...
function generateColorTheme(colors) {
    ...
    if (midColors.length >= 3) {
        ...
        if (isDark) {
            const rgb = hexToRgb(darkestColor).map(c => Math.min(c + 30, 255));
            border = rgbToHex(rgb[0] / 255, rgb[1] / 255, rgb[2] / 255);
        }
        else {
            const rgb = hexToRgb(lightestColor).map(c => Math.max(c - 30, 0));
            border = rgbToHex(rgb[0] / 255, rgb[1] / 255, rgb[2] / 255);
        }
    }
    else {
...
//# sourceMappingURL=PDFColorExtractor.js.map