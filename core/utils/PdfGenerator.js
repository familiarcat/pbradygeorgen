...
exports.generatePdfFromElement = generatePdfFromElement;
exports.generatePdfFromMarkdown = generatePdfFromMarkdown;
exports.generatePdfDataUrlFromMarkdown = generatePdfDataUrlFromMarkdown;
 ...
 */
function parseMarkdownForPdf(markdownContent, skipFirstHeading = false) {
    const blocks = [];
    ...
    const cleanMarkdownFormatting = (text) => {
        ...
        try {
            ...
            DanteLogger_1.DanteLogger.error.runtime(`Error cleaning markdown formatting: ${error}`);
 ...
 */
async function generatePdfFromElement(element, options = {}) {
    try {
        DanteLogger_1.DanteLogger.success.basic('Starting PDF generation from HTML element');
        ...
        const textColor = computedStyle.getPropertyValue('--text-color').trim() ||
            ...
            '#3A4535';
        const fontBody = computedStyle.getPropertyValue('--font-body').trim() ||
            computedStyle.getPropertyValue('--pdf-body-font').trim() ||
            computedStyle.getPropertyValue('--dynamic-primary-font').trim() ||
            'Garamond, Times New Roman, serif';
        const fontHeading = computedStyle.getPropertyValue('--font-heading').trim() ||
    ...
    `;
        ...
        const canvas = await (0, html2canvas_1.default)(container, {
        ...
        const pdf = new jspdf_1.jsPDF(mergedOptions.orientation || 'portrait', 'mm', mergedOptions.pageSize || 'a4');
        ...
        DanteLogger_1.DanteLogger.success.ux(`PDF generated and downloaded as ${mergedOptions.fileName || 'summary.pdf'}`);
        return Promise.resolve();
    }
    catch (error) {
        DanteLogger_1.DanteLogger.error.runtime(`Error generating PDF: ${error}`);
...
async function generatePdfFromMarkdown(markdownContent, options = {}) {
    try {
        DanteLogger_1.DanteLogger.success.basic('Starting PDF generation from markdown content');
        // Get CSS variables from the document with fallbacks for all naming conventions
        const computedStyle = getComputedStyle(document.documentElement);
        // Try multiple variable names for each style property with fallbacks
        const bgColor = computedStyle.getPropertyValue('--bg-primary').trim() ||
        ...
        if (options.headerText) {
            ...
            const getTextColorRGB = (hexColor) => {
                const hex = hexColor.replace('#', '');
                return {
                    r: parseInt(hex.substring(0, 2), 16),
                    g: parseInt(hex.substring(2, 4), 16),
        ...
        DanteLogger_1.DanteLogger.success.ux(`PDF generated and downloaded as ${options.fileName || 'summary.pdf'}`);
        return Promise.resolve();
    }
    catch (error) {
        DanteLogger_1.DanteLogger.error.runtime(`Error generating PDF from markdown: ${error}`);
        return Promise.reject(error);
    }
}
/**
 ...
 */
async function generatePdfDataUrlFromMarkdown(markdownContent, options = {}) {
    try {
        DanteLogger_1.DanteLogger.success.basic('Generating PDF data URL from markdown content');
        // Get CSS variables from the document with fallbacks for all naming conventions
        const computedStyle = getComputedStyle(document.documentElement);
        // Try multiple variable names for each style property with fallbacks
        const bgColor = computedStyle.getPropertyValue('--bg-primary').trim() ||
        ...
        DanteLogger_1.DanteLogger.success.ux('Generated PDF data URL');
        return dataUrl;
    }
    catch (error) {
