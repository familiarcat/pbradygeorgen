...
exports.formatSummaryContent = formatSummaryContent;
const openai_1 = __importDefault(require("openai"));
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const cacheService_1 = require("./cacheService");
const stringCacheService_1 = require("./stringCacheService");
 ...
 */
async function analyzeResume(resumeContent, forceRefresh = false) {
    try {
        ...
        const cacheKey = cacheService_1.cacheService.generateCacheKey(resumeContent);
        // Check if we have a cached response and aren't forcing a refresh
        if (!forceRefresh) {
            const cachedResponse = cacheService_1.cacheService.getItem(cacheKey);
            if (cachedResponse) {
                HesseLogger_1.HesseLogger.cache.hit(`Using cached OpenAI response for key: ${cacheKey.substring(0, 8)}...`);
                return cachedResponse;
            }
            HesseLogger_1.HesseLogger.cache.miss(`Cache miss for key: ${cacheKey.substring(0, 8)}...`);
        }
        else {
            HesseLogger_1.HesseLogger.cache.invalidate(`Force refresh requested, skipping cache for key: ${cacheKey.substring(0, 8)}...`);
        }
    ...
    `;
        ...
        HesseLogger_1.HesseLogger.openai.request(`Sending request to OpenAI for resume analysis (${resumeContent.length} chars)`);
        ...
        HesseLogger_1.HesseLogger.openai.response(`Received response from OpenAI in ${apiResponseTime}ms`);
        // Log token usage if available
        if (response.usage) {
            HesseLogger_1.HesseLogger.openai.tokens(`Token usage: ${response.usage.prompt_tokens} prompt + ${response.usage.completion_tokens} completion = ${response.usage.total_tokens} total`);
        }
        // Parse the response
        const content = response.choices[0]?.message?.content;
        if (!content) {
            HesseLogger_1.HesseLogger.openai.error("No content in the OpenAI response");
            throw new Error("No content in the OpenAI response");
        }
        console.log('Raw OpenAI response content:', content);
        HesseLogger_1.HesseLogger.summary.progress("Received raw content from OpenAI, parsing...");
        ...
        try {
            ...
            if (analysis.summary.length < 50)
                validationIssues.push('Summary too short');
            if (analysis.keySkills.length < 3)
                validationIssues.push('Too few skills');
            if (analysis.careerHighlights.length < 2)
                validationIssues.push('Too few career highlights');
            if (analysis.recommendations.length < 2)
                validationIssues.push('Too few recommendations');
            ...
            const hasThirdPerson = thirdPersonPatterns.some(pattern => pattern.test(analysis.summary) ||
            ...
            if (hasThirdPerson)
                validationIssues.push('Contains third-person language');
            ...
            cacheService_1.cacheService.setItem(cacheKey, analysis);
            HesseLogger_1.HesseLogger.cache.update(`Stored item with key: ${cacheKey.substring(0, 8)}...`);
            // Log success
            HesseLogger_1.HesseLogger.summary.complete(`Successfully generated and parsed summary`);
            return analysis;
        }
        catch (error) {
            ...
            HesseLogger_1.HesseLogger.openai.error(`Failed to parse OpenAI response: ${parseError.message || 'Unknown error'}`);
            HesseLogger_1.HesseLogger.summary.error(`JSON parsing error: ${parseError.message}`);
            throw new Error(`Failed to parse OpenAI response: ${parseError.message || 'Unknown error'}`);
        }
    }
    catch (error) {
        console.error("Error analyzing resume with OpenAI:", error);
        HesseLogger_1.HesseLogger.openai.error(`Error analyzing resume with OpenAI: ${error instanceof Error ? error.message : String(error)}`);
        HesseLogger_1.HesseLogger.summary.error(`Analysis failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
/**
 ...
 */
async function formatSummaryContent(resumeContent, forceRefresh = false) {
    try {
        ...
        // Check if we have a cached response and aren't forcing a refresh
        if (!forceRefresh) {
            const cachedResponse = stringCacheService_1.stringCacheService.getItem(cacheKey);
            if (cachedResponse) {
                HesseLogger_1.HesseLogger.cache.hit(`Using cached formatted summary for key: ${cacheKey.substring(0, 8)}...`);
                return cachedResponse;
            }
            HesseLogger_1.HesseLogger.cache.miss(`Cache miss for formatted summary key: ${cacheKey.substring(0, 8)}...`);
        }
        else {
            HesseLogger_1.HesseLogger.cache.invalidate(`Force refresh requested, skipping cache for formatted summary key: ${cacheKey.substring(0, 8)}...`);
        }
        // Read the prompt template
        const promptPath = path_1.default.join(process.cwd(), 'prompts/summary_format_prompt.txt');
        let promptTemplate = '';
        try {
            if (fs_1.default.existsSync(promptPath)) {
                promptTemplate = fs_1.default.readFileSync(promptPath, 'utf8');
            }
            else {
...
Format the content as a complete markdown document with the title "# P. Brady Georgen - Summary" at the top. Ensure all sections are properly formatted with appropriate headers, bullet points, and paragraph breaks.`;
            ...
            HesseLogger_1.HesseLogger.ai.error(`Error reading prompt template: ${error}`);
        ...
        const prompt = promptTemplate.includes('{content}')
            ...
            : promptTemplate;
        // Log the OpenAI request
        HesseLogger_1.HesseLogger.openai.request('Sending request to OpenAI for summary formatting');
        // Start timing the request
        const apiStartTime = Date.now();
        // Call the OpenAI API
        const response = await openai.chat.completions.create({
        ...
        stringCacheService_1.stringCacheService.setItem(cacheKey, content);
        HesseLogger_1.HesseLogger.cache.update(`Stored formatted summary with key: ${cacheKey.substring(0, 8)}...`);
        // Log success
        HesseLogger_1.HesseLogger.summary.complete(`Successfully formatted summary`);
        return content;
    }
    catch (error) {
        HesseLogger_1.HesseLogger.openai.error(`Error formatting summary with OpenAI: ${error instanceof Error ? error.message : String(error)}`);
        HesseLogger_1.HesseLogger.summary.error(`Formatting failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
exports.default = {
