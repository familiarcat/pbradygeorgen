...
function TaskManager() {
    const [tasks, setTasks] = (0, react_1.useState)(mockData_1.mockTasksData);
    const [activeFilter, setActiveFilter] = (0, react_1.useState)('all');
    const [activeRealm, setActiveRealm] = (0, react_1.useState)('all');
    const handleFilterChange = (filter) => {
        setActiveFilter(filter);
        DanteLogger_1.DanteLogger.success.ux(`Filtered tasks by status: ${filter}`);
    };
    const handleRealmChange = (realm) => {
        setActiveRealm(realm);
        DanteLogger_1.DanteLogger.success.ux(`Filtered tasks by realm: ${realm}`);
    };
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => handleFilterChange('all')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'all'
            ...
            : 'bg-[var(--bg-secondary, #f0f0f0)] text-[var(--text-secondary, #666)] hover:bg-[var(--bg-tertiary, #e0e0e0)]'}`}>
              ...
              <button onClick={() => handleFilterChange('todo')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'todo'
            ? 'bg-gray-700 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                📋 To Do
              </button>
              <button onClick={() => handleFilterChange('in-progress')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'in-progress'
            ? 'bg-blue-700 text-white'
            : 'bg-blue-100 text-blue-700 hover:bg-blue-200'}`}>
                🔄 In Progress
              </button>
              <button onClick={() => handleFilterChange('review')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'review'
            ? 'bg-purple-700 text-white'
            : 'bg-purple-100 text-purple-700 hover:bg-purple-200'}`}>
                👀 Review
              </button>
              <button onClick={() => handleFilterChange('completed')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'completed'
            ? 'bg-green-700 text-white'
            : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
                ✅ Completed
              </button>
              <button onClick={() => handleFilterChange('blocked')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeFilter === 'blocked'
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => handleRealmChange('all')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeRealm === 'all'
            ? 'bg-[var(--cta-primary, #0070f3)] text-white'
            : 'bg-[var(--bg-secondary, #f0f0f0)] text-[var(--text-secondary, #666)] hover:bg-[var(--bg-tertiary, #e0e0e0)]'}`}>
                All Realms
              </button>
              <button onClick={() => handleRealmChange('inferno')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeRealm === 'inferno'
            ? 'bg-red-700 text-white'
            : 'bg-red-100 text-red-700 hover:bg-red-200'}`}>
                🔥 Inferno
              </button>
              <button onClick={() => handleRealmChange('purgatorio')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeRealm === 'purgatorio'
            ? 'bg-yellow-700 text-white'
            : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200'}`}>
                ⛰️ Purgatorio
              </button>
              <button onClick={() => handleRealmChange('paradiso')} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${activeRealm === 'paradiso'
            ? 'bg-green-700 text-white'
            : 'bg-green-100 text-green-700 hover:bg-green-200'}`}>
                ✨ Paradiso
              </button>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredTasks.map((task, index) => (<tr key={index}>
