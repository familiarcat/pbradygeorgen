...
exports.default = TeamBoard;
const react_1 = require("react");
const mockData_1 = require("./mockData");
function TeamBoard() {
    const [teamData, setTeamData] = (0, react_1.useState)(mockData_1.mockTeamData);
    const getRealmEmoji = (realm) => {
        switch (realm) {
            case 'inferno': return '🔥';
    ...
    const getLevelName = (realm, level) => {
        if (realm === 'inferno') {
            const circles = [
    ...
    const getLevelEmoji = (realm, level) => {
        if (realm === 'inferno') {
            const emojis = ['🧩', '🔄', '🔋', '💾', '⚡', '⚙️', '🔨', '🔒', '💥'];
    ...
    return (<div className="space-y-8">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {teamData.members.map((member, index) => (<div key={index} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="space-y-2">
                  {member.tasks.map((task, i) => (<div key={i} className={`p-3 rounded-lg border ${getRealmColor(task.realm)}`}>
              ...
              <div className="p-4 bg-gray-50 border-t border-gray-200">
                ...
                <div className="flex h-4 rounded-full overflow-hidden">
                  <div className="bg-red-500" style={{ width: `${member.distribution.inferno}%` }} title={`Inferno: ${member.distribution.inferno}%`}></div>
                  <div className="bg-yellow-500" style={{ width: `${member.distribution.purgatorio}%` }} title={`Purgatorio: ${member.distribution.purgatorio}%`}></div>
                  <div className="bg-green-500" style={{ width: `${member.distribution.paradiso}%` }} title={`Paradiso: ${member.distribution.paradiso}%`}></div>
                </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <div className="flex items-center mb-2">
              <div className="w-full bg-blue-200 rounded-full h-3 mr-2">
                <div className="bg-blue-600 h-3 rounded-full" style={{ width: `${teamData.philosophy.hesse}%` }}></div>
              </div>
            ...
            <div className="text-xs text-blue-700">
              {teamData.philosophy.hesse < 33 ? ("The team could benefit from more technical precision and analytical thinking.") : teamData.philosophy.hesse < 66 ? ("The team has a good balance of technical precision in their work.") : ("The team excels at technical precision and analytical problem-solving.")}
            </div>
          </div>
          ...
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            ...
            <div className="flex items-center mb-2">
              <div className="w-full bg-green-200 rounded-full h-3 mr-2">
                <div className="bg-green-600 h-3 rounded-full" style={{ width: `${teamData.philosophy.salinger}%` }}></div>
              </div>
            ...
            <div className="text-xs text-green-700">
              {teamData.philosophy.salinger < 33 ? ("The team could focus more on authentic user experiences and human needs.") : teamData.philosophy.salinger < 66 ? ("The team has a good balance of human-centered design in their work.") : ("The team excels at creating authentic, human-centered experiences.")}
            </div>
          </div>
          ...
          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
            ...
            <div className="flex items-center mb-2">
              <div className="w-full bg-purple-200 rounded-full h-3 mr-2">
                <div className="bg-purple-600 h-3 rounded-full" style={{ width: `${teamData.philosophy.derrida}%` }}></div>
              </div>
            ...
            <div className="text-xs text-purple-700">
              {teamData.philosophy.derrida < 33 ? ("The team could benefit from more questioning of assumptions and conventions.") : teamData.philosophy.derrida < 66 ? ("The team has a good balance of critical thinking in their work.") : ("The team excels at challenging assumptions and finding innovative solutions.")}
            </div>
          </div>
          ...
          <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
            ...
            <div className="flex items-center mb-2">
              <div className="w-full bg-amber-200 rounded-full h-3 mr-2">
                <div className="bg-amber-600 h-3 rounded-full" style={{ width: `${teamData.philosophy.dante}%` }}></div>
              </div>
            ...
            <div className="text-xs text-amber-700">
              {teamData.philosophy.dante < 33 ? ("The team could benefit from a more structured approach to overcoming challenges.") : teamData.philosophy.dante < 66 ? ("The team has a good balance of perseverance and growth in their work.") : ("The team excels at navigating challenges and achieving excellence.")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4">
          {teamData.learningActivities.map((activity, index) => (<div key={index} className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {activity.participants.map((participant, i) => (<div key={i} className="px-2 py-1 bg-indigo-100 rounded-full text-xs font-medium text-indigo-800">
                        {participant}
                      </div>))}
                  </div>
