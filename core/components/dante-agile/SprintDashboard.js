...
exports.default = SprintDashboard;
const react_1 = require("react");
const DanteLogger_1 = require("@/utils/DanteLogger");
const mockData_1 = require("./mockData");
function SprintDashboard() {
    const [currentSprint, setCurrentSprint] = (0, react_1.useState)(mockData_1.mockSprintData);
    ...
    const handleDayClick = (day) => {
        ...
        const date = new Date(sprintStart);
        date.setDate(date.getDate() + day);
        DanteLogger_1.DanteLogger.success.ux(`Viewing tasks for ${date.toLocaleDateString()}`);
    ...
    return (<div className="space-y-8">
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="flex items-center mb-6">
          <div className="w-full bg-gray-200 rounded-full h-4 mr-4">
            <div className="bg-[var(--cta-primary, #0070f3)] h-4 rounded-full" style={{ width: `${progressPercentage}%` }}></div>
          </div>
          ...
          <div className="bg-red-50 rounded-lg p-4 border border-red-200">
            ...
            <div className="flex items-center">
              <div className="w-full bg-red-200 rounded-full h-3 mr-2">
                <div className="bg-red-600 h-3 rounded-full" style={{ width: `${infernoPercentage}%` }}></div>
              </div>
          ...
          <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
            ...
            <div className="flex items-center">
              <div className="w-full bg-yellow-200 rounded-full h-3 mr-2">
                <div className="bg-yellow-600 h-3 rounded-full" style={{ width: `${purgatorioPercentage}%` }}></div>
              </div>
          ...
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            ...
            <div className="flex items-center">
              <div className="w-full bg-green-200 rounded-full h-3 mr-2">
                <div className="bg-green-600 h-3 rounded-full" style={{ width: `${paradisoPercentage}%` }}></div>
              </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, i) => (<div key={i} className="text-center text-sm font-medium text-[var(--text-tertiary, #888)]">
              {day}
            </div>))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {Array.from({ length: 14 }).map((_, i) => {
            ...
            return (<button key={i} onClick={() => handleDayClick(i)} className={`
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentSprint.features.map((feature, index) => (<tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {feature.name}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex -space-x-2">
                      {feature.teamMembers.map((member, i) => (<div key={i} className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-medium border-2 border-white" title={member.name}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex items-center">
                      <div className="w-full bg-red-100 rounded-full h-2 mr-2">
                        <div className="bg-red-600 h-2 rounded-full" style={{ width: `${feature.progress.inferno}%` }}></div>
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex items-center">
                      <div className="w-full bg-yellow-100 rounded-full h-2 mr-2">
                        <div className="bg-yellow-600 h-2 rounded-full" style={{ width: `${feature.progress.purgatorio}%` }}></div>
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex items-center">
                      <div className="w-full bg-green-100 rounded-full h-2 mr-2">
                        <div className="bg-green-600 h-2 rounded-full" style={{ width: `${feature.progress.paradiso}%` }}></div>
                      </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <div className="flex items-center">
                      <div className="w-full bg-blue-100 rounded-full h-2 mr-2">
                        <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${feature.progress.overall}%` }}></div>
                      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-4">
              {currentSprint.pairSessions.map((session, index) => (<div key={index} className="flex items-center p-3 bg-blue-50 rounded-lg border border-blue-100">
          ...
          <div>
            ...
            <div className="space-y-4">
              {currentSprint.mobSessions.map((session, index) => (<div key={index} className="flex items-center p-3 bg-purple-50 rounded-lg border border-purple-100">
                  <div className="flex -space-x-2 mr-4">
                    {session.members.map((member, i) => (<div key={i} className="w-8 h-8 rounded-full bg-purple-200 flex items-center justify-center text-xs font-medium border-2 border-purple-50">
                        {member.name.split(' ').map(n => n[0]).join('')}
                      </div>))}
                  </div>
                  <div>
...
//# sourceMappingURL=SprintDashboard.js.map