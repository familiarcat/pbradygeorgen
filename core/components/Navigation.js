...
function Navigation({ showPDFInfo = true }) {
    const pathname = (0, navigation_1.usePathname)();
    const [isMenuOpen, setIsMenuOpen] = (0, react_1.useState)(false);
    const [pdfInfo, setPdfInfo] = (0, react_1.useState)(null);
      ...
      <div className="nav-container">
        ...
        <div className={`nav-links ${isMenuOpen ? 'open' : ''}`}>
          {navLinks
            ...
            .map((link) => (<link_1.default key={link.href} href={link.href} className={pathname === link.href ? 'active' : ''}>
                {link.label}
              </link_1.default>))}
        </div>

        {showPDFInfo && pdfInfo && (<div className="pdf-info">
            {pdfInfo.name}
          </div>)}
      </div>
