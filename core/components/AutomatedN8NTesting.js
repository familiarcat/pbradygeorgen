...
exports.AutomatedN8NTesting = AutomatedN8NTesting;
const react_1 = require("react");
const ThemeProvider_1 = require("@/theme/ThemeProvider");
const ComponentLibrary_1 = require("@/theme/ComponentLibrary");
function AutomatedN8NTesting({ crewMembers, onTestResult }) {
    const theme = (0, ThemeProvider_1.useTheme)();
    const [isRunning, setIsRunning] = (0, react_1.useState)(false);
    const [testResults, setTestResults] = (0, react_1.useState)([]);
    const [selectedScenarios, setSelectedScenarios] = (0, react_1.useState)([]);
    const [selectedCrew, setSelectedCrew] = (0, react_1.useState)([]);
    const [progress, setProgress] = (0, react_1.useState)({ current: 0, total: 0 });
    ...
    const handleScenarioToggle = (scenarioId) => {
        setSelectedScenarios(prev => prev.includes(scenarioId)
    ...
    const runAutomatedTests = async () => {
        ...
        for (let i = 0; i < testCombinations.length; i++) {
            ...
            try {
                ...
                setTestResults(prev => prev.map(r => r.id === testResult.id ? updatedResult : r));
    ...
    const getScenarioCategoryColor = (category) => {
        const colors = {
    ...
    return (<ComponentLibrary_1.Card variant="primary">
            <ComponentLibrary_1.Flex direction="column" gap={theme.spacing.lg}>
                ...
                <div>
                    <ComponentLibrary_1.Flex align="center" justify="space-between" style={{ marginBottom: theme.spacing.md }}>
                        <ComponentLibrary_1.Text variant="h4" color="primary">
                            Test Scenarios ({selectedScenarios.length}/{testScenarios.length})
                        </ComponentLibrary_1.Text>
                        <ComponentLibrary_1.Flex gap={theme.spacing.sm}>
                            <ComponentLibrary_1.Button variant="secondary" size="sm" onClick={selectAllScenarios}>
                                Select All
                            </ComponentLibrary_1.Button>
                            <ComponentLibrary_1.Button variant="secondary" size="sm" onClick={() => setSelectedScenarios([])}>
                                Clear All
                            </ComponentLibrary_1.Button>
                    ...
                    <ComponentLibrary_1.Grid columns={3} gap={theme.spacing.md}>
                        {testScenarios.map((scenario) => (<div key={scenario.id} style={{
                cursor: 'pointer',
                ...
                transition: theme.transitions.normal,
                                ...
                                <ComponentLibrary_1.Card variant="primary">
                                    <ComponentLibrary_1.Flex direction="column" gap={theme.spacing.sm}>
                                        ...
                                        <ComponentLibrary_1.Flex gap={theme.spacing.xs}>
                                            <ComponentLibrary_1.Badge variant="primary" size="sm">
                                                {scenario.category.replace('_', ' ')}
                                            </ComponentLibrary_1.Badge>
                                            <ComponentLibrary_1.Badge variant="secondary" size="sm" style={{ color: getComplexityColor(scenario.complexity) }}>
                                                {scenario.complexity}
                                            </ComponentLibrary_1.Badge>
                                        </ComponentLibrary_1.Flex>
                                    </ComponentLibrary_1.Flex>
                ...
                <div>
                    <ComponentLibrary_1.Flex align="center" justify="space-between" style={{ marginBottom: theme.spacing.md }}>
                        ...
                        <ComponentLibrary_1.Flex gap={theme.spacing.sm}>
                            <ComponentLibrary_1.Button variant="secondary" size="sm" onClick={selectAllCrew}>
                                Select All
                            </ComponentLibrary_1.Button>
                            <ComponentLibrary_1.Button variant="secondary" size="sm" onClick={() => setSelectedCrew([])}>
                                Clear All
                            </ComponentLibrary_1.Button>
                        </ComponentLibrary_1.Flex>
                    </ComponentLibrary_1.Flex>
                ...
                {/* Test Controls */}
                <div style={{ textAlign: 'center' }}>
                    <ComponentLibrary_1.Flex gap={theme.spacing.md} justify="center">
                        <ComponentLibrary_1.Button variant="primary" size="lg" onClick={runAutomatedTests} disabled={isRunning || selectedCrew.length === 0 || selectedScenarios.length === 0}>
                            {isRunning ? (<ComponentLibrary_1.Flex align="center" gap={theme.spacing.sm}>
                                    <div style={{
                width: '16px',
                height: '16px',
                ...
                animation: 'spin 1s linear infinite'
                        ...
                        <ComponentLibrary_1.Button variant="secondary" size="lg" onClick={clearAll} disabled={isRunning}>
                            Clear All
                        </ComponentLibrary_1.Button>
                    </ComponentLibrary_1.Flex>
                </div>
                ...
                {testResults.length > 0 && (<div>
                        <ComponentLibrary_1.Flex align="center" justify="space-between" style={{ marginBottom: theme.spacing.md }}>
                            ...
                            <ComponentLibrary_1.Flex gap={theme.spacing.sm}>
                                <ComponentLibrary_1.Badge variant="success" size="sm">
                                    ✅ {successCount} Success
                                </ComponentLibrary_1.Badge>
                                <ComponentLibrary_1.Badge variant="error" size="sm">
                                    ❌ {errorCount} Errors
                                </ComponentLibrary_1.Badge>
                                {runningCount > 0 && (<ComponentLibrary_1.Badge variant="warning" size="sm">
                ...
                return (<ComponentLibrary_1.Card key={result.id} variant="primary">
                                        <ComponentLibrary_1.Flex direction="column" gap={theme.spacing.sm}>
                                            <ComponentLibrary_1.Flex align="center" justify="space-between">
                                                <ComponentLibrary_1.Text variant="h4" color="primary">
                                                    {crewMember?.name} - {scenario?.name}
                                                </ComponentLibrary_1.Text>
                                                <ComponentLibrary_1.Badge variant={result.status === 'success' ? 'success' : result.status === 'error' ? 'error' : 'warning'} size="sm">
                                            ...
                                            {result.error && (<ComponentLibrary_1.Text variant="caption" style={{ color: theme.colors.error }}>
                                                    Error: {result.error}
                                                </ComponentLibrary_1.Text>)}

                                            {result.response?.testMetrics?.n8nStatus && (<ComponentLibrary_1.Text variant="caption" color="secondary">
...
//# sourceMappingURL=AutomatedN8NTesting.js.map