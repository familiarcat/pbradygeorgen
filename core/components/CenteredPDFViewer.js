...
exports.default = CenteredPDFViewer;
const react_1 = require("react");
const PDFAnalyzer_1 = __importDefault(require("./PDFAnalyzer"));
const DynamicThemeProvider_1 = __importDefault(require("./DynamicThemeProvider"));
const SalingerHeader_1 = __importDefault(require("./SalingerHeader"));
const UploadModal_1 = __importDefault(require("./UploadModal"));
const DanteLogger_1 = require("@/utils/DanteLogger");
function CenteredPDFViewer({ pdfUrl, pdfName }) {
    const showControls = false;
    const [showUI, setShowUI] = (0, react_1.useState)(false);
    const [uiTimeout, setUiTimeout] = (0, react_1.useState)(null);
    const iframeRef = (0, react_1.useRef)(null);
    // Loading state management
    const [pdfLoaded, setPdfLoaded] = (0, react_1.useState)(false);
    const [initialAnimationComplete, setInitialAnimationComplete] = (0, react_1.useState)(false);
    const [pdfVisible, setPdfVisible] = (0, react_1.useState)(false);
    // PDF analysis toggle
    const [showAnalyzer, setShowAnalyzer] = (0, react_1.useState)(false);
    // Upload modal state
    ...
    const handlePdfLoad = () => {
        DanteLogger_1.DanteLogger.success.ux('PDF loaded successfully');
        setPdfLoaded(true);
    ...
    const handleDownload = () => {
        DanteLogger_1.DanteLogger.success.ux('Download button clicked');
        // Download functionality is handled in the SalingerHeader component
    };
    // Handle view summary action
    const handleViewSummary = () => {
        setShowAnalyzer(true);
        DanteLogger_1.DanteLogger.success.ux('Summary view opened');
    };
    // Handle contact action
    const handleContact = () => {
        DanteLogger_1.DanteLogger.success.ux('Contact button clicked');
    ...
    const handleUpload = () => {
        ...
        DanteLogger_1.DanteLogger.success.ux('Upload modal opened');
    };
    // Handle PDF uploaded
    ...
    const [pdfWidth, setPdfWidth] = (0, react_1.useState)('80%');
    ...
    const [headerHeight, setHeaderHeight] = (0, react_1.useState)('5rem');
    ...
    return (<DynamicThemeProvider_1.default pdfUrl={pdfUrl}>
      <div className="relative w-full min-h-screen h-full overflow-hidden flex flex-col" style={{
            backgroundColor: 'var(--bg-primary, #d4d1be)', // Use CSS variable with fallback
            minHeight: 'calc(var(--vh, 1vh) * 100)'
        }}>
        {/* Salinger Header */}
        <SalingerHeader_1.default onDownload={handleDownload} onViewSummary={handleViewSummary} onContact={handleContact} onUpload={handleUpload} fileName={baseFileName}/>

        {/* Upload Modal */}
        <UploadModal_1.default isOpen={showUploadModal} onClose={() => setShowUploadModal(false)} onPdfUploaded={handlePdfUploaded}/>

        {/* Loading indicator - shown until PDF is loaded */}
        {!pdfVisible && (<div className="absolute inset-0 flex justify-center items-center z-20" style={{ backgroundColor: 'var(--bg-primary, #d4d1be)' }}>
        ...
        {showAnalyzer && (<div className="absolute top-4 left-4 z-20 w-[32rem] max-w-[90vw]">
            <PDFAnalyzer_1.default onClose={() => setShowAnalyzer(false)}/>
          </div>)}

        {/* PDF Viewer Container - takes remaining vertical space */}
        <div className="flex-grow flex flex-col overflow-hidden transition-all duration-1500 ease-in-out" style={{
            backgroundColor: 'var(--bg-primary, #d4d1be)', // Use CSS variable with fallback
          ...
          <div className="mx-auto rounded-lg overflow-hidden shadow-lg flex-grow pdf-container" style={{
            backgroundColor: 'var(--bg-primary, #d4d1be)', // Use CSS variable with fallback
            width: pdfWidth,
            ...
            <iframe ref={iframeRef} src={`${pdfUrl}#toolbar=0&navpanes=0&scrollbar=1&view=FitH`} className="w-full h-full flex-grow pdf-iframe" style={{
            border: 'none',
            backgroundColor: 'var(--bg-primary, #d4d1be)', // Use CSS variable with fallback
            margin: 0,
            padding: 0,
            ...
            width: '100%',
            height: '100%'
        }} title="PDF Viewer" onLoad={handlePdfLoad}/>
          </div>
        </div>
      </div>
    </DynamicThemeProvider_1.default>);
}
//# sourceMappingURL=CenteredPDFViewer.js.map