...
function PDFViewer({ pdfUrl, pdfName }) {
    ...
    const [showUI, setShowUI] = (0, react_1.useState)(false); // Start with UI hidden
    ...
    (0, react_1.useEffect)(() => {
        ...
        const handlePdfLoad = () => {
            ...
            if (pdfLoaded)
                return;
            setPdfLoaded(true);
            DanteLogger_1.DanteLogger.success.basic('PDF loaded successfully', { pdfUrl });
    ...
    const handleDownload = () => {
        ...
        DanteLogger_1.DanteLogger.success.ux('PDF downloaded', { fileName: pdfName });
    };
    ...
    const handleContact = () => {
        ...
        DanteLogger_1.DanteLogger.success.basic('Contact action triggered');
    };
    ...
    return (<DynamicThemeProvider_1.default pdfUrl={pdfUrl}>
      <div className="relative w-full h-screen overflow-hidden" style={{ backgroundColor: 'var(--bg-primary)' }}>
        ...
        {!pdfVisible && (<div className="absolute inset-0 flex justify-center items-center z-20 bg-[var(--bg-primary)]">
        ...
        <div className="w-full h-full pdf-container transition-all duration-1500 ease-in-out" style={{
            margin: 0,
            padding: 0,
            opacity: pdfVisible ? 1 : 0,
            transform: pdfVisible ? 'scale(1)' : 'scale(0.98)'
        }}>
          <iframe ref={iframeRef} src={`${pdfUrl}#toolbar=0&navpanes=0&scrollbar=0&view=FitH`} className="w-full h-[calc(100vh-4rem)] pdf-iframe mt-16" style={{
            border: 'none',
